...
public class TwinEqual extends AbstractConstraint {
    ...
    public List<ForbiddenRegion> getForbiddenRegions() {
        ...
        for(int vx = x.min(); vx <= x.max(); vx++){
            double validY = (double)(result - cofX * vx) / cofY;  // The only y value that satisfies x + y = 2
            if (validY % 1 != 0) {
                // validY is not an integer, so all y values are forbidden for this x
                regions.add(new ForbiddenRegion(vx, vx, minY, maxY));
                continue;
            }
            else {
                int validYInt = (int) validY;
                
                // Create forbidden region for y < validY
                if (minY < validYInt) {
                    regions.add(new ForbiddenRegion(vx, vx, minY, Math.min(validYInt - 1, maxY)));
                }

                // Create forbidden region for y > validY
                if (maxY > validYInt) {
                    regions.add(new ForbiddenRegion(vx, vx,  Math.max(validYInt + 1, minY), maxY));
                }
            }
        }
